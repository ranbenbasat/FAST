...
A customization of Cormode's Count Min implementation for implementing Count Sketch


*****************************/

CS_type * CS_Init(int width, int depth, int seed)
{     // Initialize the sketch based on user-supplied size
	CS_type * cs;
	int j;
	prng_type * prng;

	cs = (CS_type *)malloc(sizeof(CS_type));
	prng = prng_Init(-abs(seed), 2);
	// initialize the generator to pick the hash functions
	...
	if (cs && prng)
	{
		...
		cs->width = width;
		cs->count = 0;
		cs->counts = (int **)calloc(sizeof(int *), cs->depth);
		cs->counts[0] = (int *)calloc(sizeof(int), cs->depth*cs->width);
		cs->hasha = (unsigned int *)calloc(sizeof(unsigned int), cs->depth);
		cs->hashb = (unsigned int *)calloc(sizeof(unsigned int), cs->depth);
		cs->signhasha = (unsigned int *)calloc(sizeof(unsigned int), cs->depth);
		cs->signhashb = (unsigned int *)calloc(sizeof(unsigned int), cs->depth);
		if (cs->counts && cs->hasha && cs->hashb && cs->counts[0])
		{
			for (j = 0; j<depth; j++)
			{
				cs->hasha[j] = prng_int(prng) & MOD;
				cs->hashb[j] = prng_int(prng) & MOD;
				cs->signhasha[j] = prng_int(prng) & MOD;
				cs->signhashb[j] = prng_int(prng) & MOD;
				// pick the hash functions
				cs->counts[j] = (int *)cs->counts[0] + (j*cs->width);
			}
		}
		else cs = NULL;
	}
...
void CS_Destroy(CS_type * cs)
{     // get rid of a sketch and free up the space
	if (!cs) return;
	if (cs->counts)
	{
		if (cs->counts[0]) free(cs->counts[0]);
		free(cs->counts);
		cs->counts = NULL;
	}
	if (cs->hasha) free(cs->hasha); cs->hasha = NULL;
	if (cs->hashb) free(cs->hashb); cs->hashb = NULL;
	free(cs);  cs = NULL;
}

inline int CS_sign(CS_type * cs, unsigned int item, int j)
{
	return 1 - 2 * hash31(cs->signhasha[j], cs->signhashb[j], item) & 1;
}

void CS_Update(CS_type * cs, unsigned int item, int diff)
{
	int j;
	...
	for (j = 0; j<cs->depth; j++)
		cs->counts[j][hash31(cs->hasha[j], cs->hashb[j], item) % cs->width] += diff * CS_sign(cs, item, j);
	// this can be done more efficiently if the width is a power of two
}

int CS_Size(CS_type * cm)
{ // return the size of the sketch in bytes
	int counts, hashes, admin;
	if (!cm) return 0;
	admin = sizeof(CM_type);
	counts = cm->width*cm->depth * sizeof(int);
	hashes = cm->depth * 4 * sizeof(unsigned int);
	return(admin + hashes + counts);
}


int CS_Query(CS_type * cs, unsigned int query)
{
	// return an estimate of the count of an item by taking the minimum
	int j, ans;

	if (!cs) return 0;
	ans = cs->counts[0][hash31(cs->hasha[0], cs->hashb[0], query) % cs->width];
	for (j = 1; j<cs->depth; j++)
		ans = min(ans, cs->counts[j][hash31(cs->hasha[j], cs->hashb[j], query) % cs->width]);
	// this can be done more efficiently if the width is a power of two
	return (ans);
}
